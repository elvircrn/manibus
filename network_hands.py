...
def get_experiment_params(data_path=data.DATA_PATH):
    return tf.contrib.training.HParams(
        ...
        architecture=arch.hands_mini_vgg,
        dropout=0.6,
        run_preprocess=True,
        data_path=data_path
    )
...
def experiment_fn(run_config, params):
    ...
    datasets = preprocess_hands.preprocess(params.data_path)
...
def get_train_inputs(batch_size, datasets):
    ...
    def train_inputs():
        with tf.name_scope(data.TRAINING_SCOPE):
            images = datasets[0][0].reshape([-1, data.HANDS_IMAGE_WIDTH, data.HANDS_IMAGE_HEIGHT, 1])
            labels = datasets[0][1]
...
def get_test_inputs(batch_size, datasets):
    ...
    def test_inputs():
        with tf.name_scope(data.TEST_SCOPE):
            images = datasets[1][0].reshape([-1, data.HANDS_IMAGE_WIDTH, data.HANDS_IMAGE_HEIGHT, 1])
            labels = datasets[1][1]
            # Define placeholders
            images_placeholder = tf.placeholder(
                images.dtype, images.shape)
...
def run_network(data_path=data.HANDS_DATA_PATH):
    enable_gpu = True
